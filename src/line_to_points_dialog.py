...
class LineToPointsDialog(QtGui.QDialog, FORM_CLASS):
    def __init__(self, parent=None):
        """Constructor."""
        super(LineToPointsDialog, self).__init__(parent)
    ...
    def convert(self, layerLine):
        ...
        layerPoint = QgsVectorLayer(path + '.shp', "sources_from_line", "ogr")
        ...
        for fet in features:
            ...
            for g in geom:
                fetPoint = QgsFeature()
                fetPoint.setGeometry(QgsGeometry.fromPoint(g))
                fetPoint.setAttributes([fet["name"], float(fet["mnozstvi"]) / float(geom_count), fet["vyska"], fet["teplota"], fet["prumer"], fet["rychlost"], fet["vyuziti"], float(fet["objem"]) / float(geom_count)])
